...
func TestUpdateList_d(t *testing.T) {
	...
	if len(m.cfg.RSSFeeds) != 1 {
		t.Errorf("expected 1 feed after delete, got %d", len(m.cfg.RSSFeeds))
	}
	if m.statusMsg == "" {
...
func TestUpdateList_dAtEnd(t *testing.T) {
	...
	if len(m.cfg.RSSFeeds) != 1 {
		t.Errorf("expected 1 feed, got %d", len(m.cfg.RSSFeeds))
	}
}
...
func TestUpdateInput_enterEmpty(t *testing.T) {
	...
	if m.statusMsg != "URL cannot be empty" {
		t.Errorf("expected empty url warning, got %q", m.statusMsg)
	}
}
...
func TestUpdateInput_enterValid(t *testing.T) {
	m := setupModel(t)
	m.mode = modeInput
	m.input = "https://example.com/new.xml"
	m, _ = testutil.UpdateModel(m, testutil.KeySpecial(tea.KeyEnter))
	if m.mode != modeList {
		t.Errorf("expected modeList, got %v", m.mode)
	}
	...
	if m.cursor != len(m.cfg.RSSFeeds)-1 {
...
func TestUpdateList_dSaveError(t *testing.T) {
	...
	m, _ = testutil.UpdateModel(m, testutil.KeyRune('d'))
	if len(m.cfg.RSSFeeds) != 2 {
		t.Fatalf("expected 2 feeds (save failed, state unchanged), got %d", len(m.cfg.RSSFeeds))
	}
}
